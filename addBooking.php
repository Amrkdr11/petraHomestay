...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        $status = 'Confirmed';  // Assuming user_id is stored in session after login

        $user_id = $_SESSION['user_id'];  // Assuming user_id is stored in session after login

        // Check for date conflicts
        $stmt = $conn->prepare("SELECT * FROM bookings WHERE (checkin <= ? AND checkout >= ?) OR (checkin <= ? AND checkout >= ?) AND booking_status = 'Confirmed'");
        ...
        if (empty($errors)) {
            ...
            $stmt = $conn->prepare("INSERT INTO bookings (checkin, checkout, guest_count, nightly_rate, cleaning_fee, subtotal, total, user_id, created_at, booking_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)");
            ...
            // Bind parameters to the statement
            $stmt->bind_param("ssiididis", $checkin, $checkout, $guest_count, $nightlyRate, $cleaningFee, $subtotal, $total, $user_id, $status);
